...
export function AgentCarrierTracker() {
  ...
  const { data: credentials = [], refetch } = trpc.agent.credentials.list.useQuery();
  const addMutation = trpc.agent.credentials.add.useMutation();
  const updateMutation = trpc.agent.credentials.update.useMutation();
  const deleteMutation = trpc.agent.credentials.delete.useMutation();
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-white">Carrier Portal Tracker</h2>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10 bg-white/3">
                  <th className="text-left text-gray-400 text-xs uppercase px-4 py-3 font-semibold tracking-wider">Carrier</th>
                  <th className="text-left text-gray-400 text-xs uppercase px-4 py-3 font-semibold tracking-wider">Writing #</th>
                  <th className="text-left text-gray-400 text-xs uppercase px-4 py-3 font-semibold tracking-wider">Username</th>
                  <th className="text-left text-gray-400 text-xs uppercase px-4 py-3 font-semibold tracking-wider">Password</th>
                  <th className="text-right text-gray-400 text-xs uppercase px-4 py-3 font-semibold tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredCredentials.map((cred) => (
                  <tr key={cred.id} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                    <td className="px-4 py-3 text-white font-semibold">{cred.carrier}</td>
                    <td className="px-4 py-3 text-gray-300 font-mono text-sm">{cred.writingNumber || "—"}</td>
                    <td className="px-4 py-3 text-gray-300 font-mono text-sm">{cred.username}</td>
